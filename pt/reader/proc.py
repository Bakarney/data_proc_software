...
def list_struct_values(struct):
    flat = []
    values = [i for i in struct.values()]
    if isinstance(values[0], dict):
        for i in values:
            flat += list_struct_values(i)
...
def read_data(summoners_mode, summoners_output,
    ...
    if details_mode != "skip":
        ...
        summoners = list_struct_values(summoners)
        summoners = [i for i in summoners if i["wins"] + i["losses"] > 100]
    ...
    if games_ids_mode != "skip":
        ...
        details = list_struct_values(details)
    ...
    if games_mode != "skip":
        ...
        games_ids = list_struct_values(games_ids)
...
def flatten_dict(data: dict, prefix=None):
    res = {}
    for k, v in data.items():
        if isinstance(v, dict):
            res.update(flatten_dict(v, prefix + "_" + k if prefix else k))
        elif isinstance(v, list):
            for i, j in enumerate(v):
                if isinstance(j, dict):
                    res.update(flatten_dict(j, prefix + "_" + k + "_" + str(i) if prefix else k))
                else:
                    res[prefix + "_" + k + "_" + str(i)] = j
        else:
            res[prefix + "_" + k] = v
    return res


def combine_data(flat_output, games_output):
    games = read_struct(games_output)[const.region]
    games = [i for i in games.values()]

    df = {k: [v] for k, v in flatten_dict(games.pop(0)).items()}
    bar = tqdm(games)
    for game in bar:
        f = flatten_dict(game)
        for k, v in f.items():
    ...
    json.dump(df, open(os.path.join(flat_output, "result.json").replace("\\", "/"), "w"))
